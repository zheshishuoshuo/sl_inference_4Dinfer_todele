import numpy as np
import pandas as pd
from tqdm import tqdm
from scipy.stats import norm

from concurrent.futures import ProcessPoolExecutor
from itertools import repeat

import os
import sys
import types

from .mock_generator.mass_sampler import generate_samples, MODEL_PARAMS
from .mock_generator.lens_solver import solve_single_lens
from .mock_generator.lens_model import LensModel
from .utils import selection_function
from .config import SCATTER

LOGMHMIN = 8
LOGMHMAX = 25

def sample_lens_population(n_samples, zl=0.3, zs=2.0):
    """Generate lens population samples without alpha offset."""
    data = generate_samples(n_samples)
    logM_star_sps = data["logM_star_sps"]
    logRe = data["logRe"]
    beta = np.random.rand(n_samples) ** 0.5
    # sample halo mass uniformly to allow importance reweighting later
    logMh_min, logMh_max = 8.0, 18.0
    logMh = np.random.uniform(logMh_min, logMh_max, n_samples)
    return {
        "logM_star_sps": logM_star_sps,
        "logRe": logRe,
        "logMh": logMh,
        "beta": beta,
        "logMh_min": logMh_min,
        "logMh_max": logMh_max,
        "zl": zl,
        "zs": zs,
    }


# def _solve_magnification(args):
#     """Helper to solve a single lens and return magnifications."""
#     logM_star, logRe, logMh, beta, zl, zs = args
#     model = LensModel(
#         logM_star=logM_star,
#         logM_halo=logMh,
#         logRe=logRe,
#         zl=zl,
#         zs=zs,
#     )
#     xA, xB = solve_single_lens(model, beta)
#     ycaustic = model.solve_ycaustic()
#     if ycaustic is None:
#         ycaustic = 0.0
#     return model.mu_from_rt(xA), model.mu_from_rt(xB), ycaustic



# AFTER
def _solve_magnification(args):
    logM_star, logRe, logMh, beta, zl, zs = args
    try:
        model = LensModel(
            logM_star=logM_star, logM_halo=logMh, logRe=logRe, zl=zl, zs=zs
        )
        xA, xB = solve_single_lens(model, beta)
        ycaustic = model.solve_ycaustic() or 0.0
        mu1 = model.mu_from_rt(xA)
        mu2 = model.mu_from_rt(xB)
        if not (np.isfinite(mu1) and np.isfinite(mu2) and ycaustic > 0):
            return (np.nan, np.nan, 0.0)
        return (mu1, mu2, ycaustic)
    except Exception:
        return (np.nan, np.nan, 0.0)


def compute_magnifications(logM_star, logRe, logMh, beta, zl, zs, n_jobs=None):
    """Solve lens equation for each sample and return magnifications and caustic size."""
    n = len(logM_star)
    args = zip(logM_star, logRe, logMh, beta, repeat(zl), repeat(zs))
    with ProcessPoolExecutor(max_workers=n_jobs) as pool:
        results = list(
            tqdm(
                pool.map(_solve_magnification, args),
                total=n,
                desc="solving lenses",
            )
        )
    mu1, mu2, ycaustic = map(np.array, zip(*results))
    return mu1, mu2, ycaustic


def ms_distribution(ms_grid, alpha_s=-1.3, ms_star=24.5):
    """Normalized PDF of source magnitude on a grid."""
    L = 10 ** (-0.4 * (ms_grid - ms_star))
    pdf = L ** (alpha_s + 1) * np.exp(-L)
    pdf /= np.trapz(pdf, ms_grid)
    return pdf


    # if not (
    #     12.0 < mu0 < 14.0
    #     and 0 < sigmaDM < 1
    #     and 0. < sigma_alpha < 1
    #     and -0.3 < mu_alpha < 0.5
    #     and 0 < beta < 5
    # ):


def build_eta_grid():
    mu_DM_grid = np.linspace(10, 16, 100)
    alpha_grid = np.linspace(0., 0.5, 100)
    return mu_DM_grid, alpha_grid


def compute_A_eta(n_samples=5000, ms_points=100, m_lim=26.5, lens_file=os.path.join(os.path.dirname(__file__), "lens_samples.csv")):
    """Compute normalization grid A(eta).

    If a cached lens sample file exists with the requested number of samples,
    reuse it instead of regenerating the lens population and recomputing
    magnifications.  The final A-table is accumulated per sample to avoid
    holding large 4D arrays in memory.
    """

    lens_df = None
    if os.path.exists(lens_file):
        cached = pd.read_csv(lens_file)
        if len(cached) == n_samples:
            lens_df = cached
            print(f"Loaded {n_samples} lens samples from {lens_file}.")

    if lens_df is None:
        samples = sample_lens_population(n_samples)
        lens_df = pd.DataFrame(
            {
                "logM_star_sps": samples["logM_star_sps"],
                "logRe": samples["logRe"],
                "logMh": samples["logMh"],
                "beta": samples["beta"],
                "zl": np.full(n_samples, samples["zl"]),
                "zs": np.full(n_samples, samples["zs"]),
            }
        )
        lens_df.to_csv(lens_file, index=False)
    else:
        samples = {
            "logM_star_sps": lens_df["logM_star_sps"].values,
            "logRe": lens_df["logRe"].values,
            "logMh": lens_df["logMh"].values,
            "beta": lens_df["beta"].values,
            "zl": lens_df.get("zl", pd.Series([0.3])).iloc[0],
            "zs": lens_df.get("zs", pd.Series([2.0])).iloc[0],
            "logMh_min": float(lens_df["logMh"].min()),
            "logMh_max": float(lens_df["logMh"].max()),
        }
    a, b = samples["logMh_min"], samples["logMh_max"]
    n_samples = samples["logMh"].size  # 你后面用到了 n_samples


    ms_grid = np.linspace(20.0, 30.0, ms_points)
    pdf_ms = ms_distribution(ms_grid)

    mu_DM_grid, alpha_grid = build_eta_grid()
    mu_grid = mu_DM_grid[:, None]
    A_accum = np.zeros((mu_DM_grid.size, alpha_grid.size))

    # Fixed halo-mass relation parameters from the default model
    MODEL_P = MODEL_PARAMS["deVauc"]
    beta_DM = MODEL_P["beta_h"]
    sigma_DM = MODEL_P["sigma_h"]

    for j, alpha in enumerate(tqdm(alpha_grid, desc="alpha loop")):
        # ==== 统一 Mh 区间与样本量（放在 alpha loop 之前）====
        # a,b 用样本的最小/最大 Mh；若字段不存在，就从数组计算
        a = float(samples["logMh_min"]) if "logMh_min" in samples else float(np.min(samples["logMh"]))
        b = float(samples["logMh_max"]) if "logMh_max" in samples else float(np.max(samples["logMh"]))
        Mh_range = max(1e-12, b - a)    # 防止 b==a
        n_samples = samples["logMh"].size

        # logM_star = samples["logM_star_sps"] + alpha
        logM_star = samples["logM_star_sps"] + alpha # + np.random.normal(0, SCATTER.star, size=n_samples)

        mu1, mu2, ycaustic = compute_magnifications(
            logM_star,
            samples["logRe"],
            samples["logMh"],
            samples["beta"],
            samples["zl"],
            samples["zs"],
        )






                # ---- 清理放大率 & 有效掩码 ----
        mu1 = np.asarray(mu1); mu2 = np.asarray(mu2); ycaustic = np.asarray(ycaustic)
        mu1 = np.nan_to_num(mu1, nan=0.0, posinf=0.0, neginf=0.0)
        mu2 = np.nan_to_num(mu2, nan=0.0, posinf=0.0, neginf=0.0)
        mu1_clip = np.clip(mu1, 1e-6, 1e6)
        mu2_clip = np.clip(mu2, 1e-6, 1e6)
        valid = (mu1_clip > 0) & (mu2_clip > 0) & np.isfinite(ycaustic) & (ycaustic > 0)

        # ---- 只在有效样本上计算选择，其余样本 p_det=0 ----
        p_det = np.zeros((n_samples, ms_grid.size))
        if np.any(valid):
            sel1 = selection_function(mu1_clip[valid, None], m_lim, ms_grid[None, :], SCATTER.mag)
            sel2 = selection_function(mu2_clip[valid, None], m_lim, ms_grid[None, :], SCATTER.mag)
            p_det[valid] = sel1 * sel2
        p_det = np.clip(np.nan_to_num(p_det, nan=0.0, posinf=0.0, neginf=0.0), 0.0, 1.0)

        # ---- 对 ms 积分 & 只给有效样本乘 ycaustic^2 ----
        w_ms = np.trapz(p_det * pdf_ms[None, :], ms_grid, axis=1)
        w_ms = np.nan_to_num(w_ms, nan=0.0, posinf=0.0, neginf=0.0)
        yc2 = np.zeros_like(w_ms); yc2[valid] = ycaustic[valid] ** 2
        w_static = w_ms * yc2

        # sel1 = selection_function(mu1[:, None], m_lim, ms_grid[None, :], SCATTER.mag)
        # sel2 = selection_function(mu2[:, None], m_lim, ms_grid[None, :], SCATTER.mag)
        # p_det = sel1 * sel2


        # # p_det = np.ones((n_samples, ms_grid.size))


        # w_ms = np.trapz(p_det * pdf_ms[None, :], ms_grid, axis=1)
        # w_static = w_ms * (ycaustic ** 2)

        # for logM_sps_i, logMh_i, w_i in zip(
        #     samples["logM_star_sps"], samples["logMh"], w_static
        # ):
            
        for logM_sps_i, logMh_i, w_i in zip(samples["logM_star_sps"], samples["logMh"], w_static):
            if not (np.isfinite(w_i) and (w_i > 0)):
                continue
            # mean = ...
            # ...

            # mean = mu_grid[:, 0] + beta_DM * (logM_sps_i - 11.4)
            # p_Mh = norm.pdf(logMh_i, loc=mean, scale=sigma_DM)
            # A_accum[:, j] += w_i * p_Mh
            mean = mu_grid[:, 0] + beta_DM * (logM_sps_i - 11.4)
            Z = norm.cdf(b, loc=mean, scale=sigma_DM) - norm.cdf(a, loc=mean, scale=sigma_DM)
            Z = np.clip(Z, 1e-12, None)
            # p_Mh = norm.pdf(logMh_i, loc=mean, scale=sigma_DM) / Z
            # # A_accum[:, j] += w_i * p_Mh
            # A_accum = np.nan_to_num(A_accum, nan=0.0, posinf=0.0, neginf=0.0)  # 新增
            # A = Mh_range * A_accum / n_samples
            # A = np.maximum(A, 1e-300)  # 新增
            p_Mh = norm.pdf(logMh_i, loc=mean, scale=sigma_DM) / Z
            A_accum[:, j] += w_i * p_Mh



    # Mh_range = samples.get("logMh_max", 15.0) - samples.get("logMh_min", 11.0)
    # Mh_range = b - a
    # A = Mh_range * A_accum / n_samples
    Mh_range = b - a
    A_accum = np.nan_to_num(A_accum, nan=0.0, posinf=0.0, neginf=0.0)  # 可留
    A = Mh_range * A_accum / n_samples
    A = np.maximum(A, 1e-300)  # 可留


    mu_flat, alpha_flat = np.meshgrid(mu_DM_grid, alpha_grid, indexing="ij")
    df = pd.DataFrame(
        {
            "mu_DM": mu_flat.ravel(),
            "alpha": alpha_flat.ravel(),
            "A": A.ravel(),
        }
    )
    path = os.path.join(os.path.dirname(__file__), "A_eta_table_alpha.csv")
    df.to_csv(path, index=False)
    return df


if __name__ == "__main__":
    compute_A_eta()
