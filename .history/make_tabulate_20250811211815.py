...
def tabulate_likelihood_grids(
    ...
    for _, row in mock_observed_data.iterrows():
        ...
        for logMh in logMh_grid:
            ...
            L3 = selection_function(mu1, m_lim=26.0, ms=MS_GRID, sigma_m=sigma_m)
            L4 = selection_function(mu2, m_lim=26.0, ms=MS_GRID, sigma_m=sigma_m)
            Lphot = np.trapz(P_MS * L1 * L2 * L3 * , MS_GRID)

            sample_factor = abs(beta) * abs(detJ) * Lphot
