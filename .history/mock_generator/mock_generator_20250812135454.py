...
def simulate_single_lens(i, samples, logalpha_sps_sample,
                        maximum_magnitude, zl, zs, nbkg_kpc2):
    ...
    lambda_i = np.pi * ycaust_kpc**2 * nbkg_kpc2
    N_i = np.random.poisson(lambda_i)
    # if N_i != 0: ycaustic = {ycaust_kpc:.2f} kpc, N_sources = {N_i}")
...
def run_mock_simulation(
    ...
    nbkg : float, optional
        Surface density of background sources in ``arcsec^-2``.
    """
    ...
    # convert source density to kpc^-2
    DA_lens_kpc = Dang(zl) * 1e3
    nbkg_kpc2 = nbkg / ((206265 / DA_lens_kpc) ** 2)
    ...
    if process is None or process == 0:
        ...
        args = [
            (i, samples, logalpha_sps_sample, maximum_magnitude, zl, zs, nbkg_kpc2)
