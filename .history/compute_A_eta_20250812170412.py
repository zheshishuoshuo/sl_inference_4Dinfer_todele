...
def integrate_beta(
    ...
    """Numerically integrate over ``beta`` for a single lens.

    The source position ``beta`` is integrated from ``0`` to ``betamax`` using
    a simple trapezoidal rule.  For each beta the detection probability is
    computed by integrating over the source magnitude distribution.

    Returns
    -------
    float
        The combined weight ``T1*T2`` for this lens sample.
    """
    ...
    try:
        ...
        betamax = model.solve_ycaustic() or 0.0
        if not (np.isfinite(betamax) and betamax > 0):
            return 0.0
    except Exception:
        return 0.0

    beta_unit = np.linspace(0.0, 1.0, beta_points)
    beta_grid = beta_unit * betamax

    mu1 = np.zeros(beta_points)
    mu2 = np.zeros(beta_points)
    valid = np.ones(beta_points, dtype=bool)

    for k, bu in enumerate(beta_unit):
        try:
            xA, xB = solve_single_lens(model, bu)
            mu1[k] = model.mu_from_rt(xA)
            mu2[k] = model.mu_from_rt(xB)
        except Exception:
            valid[k] = False

    mu1 = np.nan_to_num(mu1, nan=0.0, posinf=0.0, neginf=0.0)
    mu2 = np.nan_to_num(mu2, nan=0.0, posinf=0.0, neginf=0.0)
    mu1 = np.clip(mu1, 1e-6, 1e6)
    mu2 = np.clip(mu2, 1e-6, 1e6)
    valid &= (mu1 > 0) & (mu2 > 0)

    p_det = np.zeros((beta_points, ms_grid.size))
    if np.any(valid):
        sel1 = selection_function(
            mu1[valid, None], m_lim, ms_grid[None, :], SCATTER.mag
        )
        sel2 = selection_function(
            mu2[valid, None], m_lim, ms_grid[None, :], SCATTER.mag
        )
        p_det[valid] = sel1 * sel2
    p_det = np.clip(
        np.nan_to_num(p_det, nan=0.0, posinf=0.0, neginf=0.0), 0.0, 1.0
    )

    T1_beta = np.trapz(p_det * p_ms[None, :], ms_grid, axis=1)
    T1_beta = np.nan_to_num(T1_beta, nan=0.0, posinf=0.0, neginf=0.0)

    integrand = T1_beta * 2.0 * beta_grid
    weight = np.trapz(integrand, beta_grid)
    return float(np.nan_to_num(weight, nan=0.0, posinf=0.0, neginf=0.0))
...
def compute_A_eta(
    ...
    for j, alpha in enumerate(tqdm(alpha_grid, desc="alpha loop")):
        ...
        weights = [
            integrate_beta(
                lm, lr, lmh, samples["zl"], samples["zs"], ms_grid, p_ms, m_lim
            )
            for lm, lr, lmh in zip(logM_star, samples["logRe"], samples["logMh"])
        ]

        # ---- T3: halo-mass relation for each muDM ----
        for logM_sps_i, logMh_i, w_i in zip(
            samples["logM_star_sps"], samples["logMh"], weights
        ):
